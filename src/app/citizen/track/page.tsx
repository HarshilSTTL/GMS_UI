...
const STATUS_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string; bg: string }> = {
  ...
  escalated: { label: 'Escalated', icon: AlertCircle, color: '#DC2626', bg: '#FEF2F2' },
  acknowledged: { label: 'Acknowledged', icon: Clock, color: '#0891B2', bg: '#ECFEFF' },
  under_review: { label: 'Under Review', icon: Clock, color: '#7C3AED', bg: '#F5F3FF' },
};
...
export default function TrackComplaint() {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      setError('Error fetching grievance. Please try again.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const statusConfig = grievance ? STATUS_CONFIG[grievance.status] || STATUS_CONFIG.pending : null;
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-[18px] font-bold text-[#0E1C2F] mb-1">Track Grievance</h1>
        <p className="text-[12px] text-[#7A8FA6]">Enter your token to check the status of your grievance</p>
      </div>

      {/* Search Box */}
      <div>
        <label className="block text-[11px] font-bold text-[#3D5068] mb-2">Grievance Token</label>
      ...
      {/* Grievance Details */}
      {grievance && statusConfig && StatusIcon && (
        <div className="bg-white rounded-xl border border-[#DDE3EE] overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4">
            ...
            <p className="text-[16px] font-bold">{grievance.token}</p>
          </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div>
              <div className="flex items-start justify-between mb-2">
                <p className="text-[13px] font-bold text-[#0E1C2F] flex-1">{grievance.title}</p>
                <div
              ...
              <p className="text-[11px] text-[#7A8FA6]">{grievance.category}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-[#E5E7EB] pt-4">
              <div>
                ...
                <p className="text-[12px] text-[#0E1C2F] mt-1">{grievance.department}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase text-[#7A8FA6]">Location</p>
                <p className="text-[12px] text-[#0E1C2F] mt-1">{grievance.location}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase text-[#7A8FA6]">Filed On</p>
                <p className="text-[12px] text-[#0E1C2F] mt-1">{new Date(grievance.submittedDate).toLocaleDateString()}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase text-[#7A8FA6]">Assigned To</p>
                <p className="text-[12px] text-[#0E1C2F] mt-1">{grievance.officer || 'Unassigned'}</p>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
              ...
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-[14px] font-bold text-blue-600">{grievance.slaDaysLeft > 0 ? `${grievance.slaDaysLeft} days` : 'Breached'}</p>
                  <p className="text-[10px] text-[#7A8FA6]">{grievance.slaDaysLeft > 0 ? 'remaining' : 'SLA time exceeded'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#F8FAFD] border border-[#E5E7EB] rounded-lg p-3">
              ...
              <p className="text-[12px] text-[#0E1C2F]">{new Date(grievance.updatedAt).toLocaleString()}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!grievance && !error && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
          ...
          <p className="text-[11px] text-[#3D5068] leading-relaxed">
            Your grievance token was provided when you submitted your complaint. It starts with &apos;GJ-&apos; followed by the year and a 5-digit number (e.g., GJ-2026-05341).
          </p>
        </div>
      )}
    </div>
